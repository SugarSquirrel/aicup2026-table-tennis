...
dA,gA,dP,gP=player_dists(nha,yA,yP)
...
for tag,mask in [("incl0",False),("excl0",True)]:
    ...
    sub.to_csv(f"../submission_player_{tag}.csv",index=False)
    out(f"[{tag}] rows={len(sub)} action_vc={dict(sub.actionId.value_counts().head(3))} point_vc={dict(sub.pointId.value_counts().head(3))} sgp_mean={sub.serverGetPoint.mean():.3f}")
