...
out("Running FULLY-AUGMENTED 5-fold OOF for LGBM+TabPFN+GRU (~15 min)...")
...
for f in range(5):
    ...
    # AUG: concat old samples (disjoint from train -> fold-safe) into all fold-safe stats
    ...
    T_=fit_trans({"_la":cat_la,"_lp":cat_lp},cat_yA,cat_yP)
    dMa_,gAg,dMp_,gPg=player_dists(cat_nh,cat_yA,cat_yP)
    cl_=fit_clusters(pd.concat([tr[trfold!=f],old],ignore_index=True))
    cA_,cP_=fit_matchup(cat_nh,cat_lh,cat_yA,cat_yP,cl_)
    def mkAP_f(Xb,nh,lh,idx):
        Ft=apply_trans({k:Xb[k].to_numpy() for k in KEY},T_); Ft.index=idx
        return pd.concat([Xb[BASE],Ft,player_feat(nh,dMa_,gAg,dMp_,gPg,idx),matchup_feat(nh,lh,cl_,cA_,cP_,dMa_,gAg,dMp_,gPg,idx)],axis=1)
    def mkAPp_f(Xb,nh,idx):
        Ft=apply_trans({k:Xb[k].to_numpy() for k in KEY},T_); Ft.index=idx
        return pd.concat([Xb[BASE],Ft,player_feat(nh,dMa_,gAg,dMp_,gPg,idx)],axis=1)
    def mkSV_f(Xb,idx):
        Ft=apply_trans({k:Xb[k].to_numpy() for k in KEY},T_); Ft.index=idx
        return pd.concat([Xb[BASE],Ft],axis=1)
    # === LGBM (action/point use matchup; server no player) ===
    Xt_AP=mkAP_f(Xa.loc[trm],nha[trm],lha[trm],Xa.index[trm])
    Xe_AP=mkAP_f(Xs.loc[evm],nhs[evm],lhs[evm],Xs.index[evm])
    Xt_S =mkSV_f(Xa.loc[trm],Xa.index[trm])
    Xe_S =mkSV_f(Xs.loc[evm],Xs.index[evm])
    ma=lgbc().fit(Xt_AP,yA[trm]);             LAc.append(align(ma.predict_proba(Xe_AP),ma.classes_,ACLS))
    mp=lgbc().fit(Xt_AP,yP[trm]);             LPc.append(align(mp.predict_proba(Xe_AP),mp.classes_,PCLS))
    mr=lgbc(False).fit(Xt_S,yR[trm]);         LRc.append(mr.predict_proba(Xe_S)[:,1])
    # === TabPFN (action ManyClass / point native / server native) on sampled-train + augmented features ===
    Xt_APp=mkAPp_f(Xs.loc[sft],nhs[sft],Xs.index[sft])
    Xe_APp=mkAPp_f(Xs.loc[sfe],nhs[sfe],Xs.index[sfe])
    Xt_Ss =mkSV_f(Xs.loc[sft],Xs.index[sft])
    Xe_Ss =mkSV_f(Xs.loc[sfe],Xs.index[sfe])
    tA=ManyClassClassifier(estimator=TabPFNClassifier(device=DEV,ignore_pretraining_limits=True),alphabet_size=10,random_state=SEED).fit(Xt_APp,eA[sft])
    TAc.append(align(tA.predict_proba(Xe_APp),tA.classes_,ACLS))
    tP=TabPFNClassifier(device=DEV,ignore_pretraining_limits=True).fit(Xt_APp,eP[sft]); TPc.append(align(tP.predict_proba(Xe_APp),tP.classes_,PCLS))
    tR=TabPFNClassifier(device=DEV,ignore_pretraining_limits=True).fit(Xt_Ss,eR[sft]);  TRc.append(tR.predict_proba(Xe_Ss)[:,1])
...
np.savez('oof_v6_fullaug.npz',LAc=LAc,LPc=LPc,LRc=LRc,TAc=TAc,TPc=TPc,TRc=TRc,GAc=GAc,GPc=GPc,GRc=GRc,YA=YA,YP=YP,YR=YR,SEEN=SEEN,RESCUED=RESCUED,COLD=COLD)
out("cached -> oof_v6_fullaug.npz")
...
out(f"\nv6 FULLY-AUGMENTED CV-B: action F1={fa_b:.4f} w={WA} β={BA:.3f}  | CV-A F1={fa_a:.4f}")
