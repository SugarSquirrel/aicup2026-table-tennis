...
class Time2Vec(nn.Module):
    """Per-scalar Time2Vec: each input scalar -> (k+1)-dim (1 linear + k periodic sin)."""
    def __init__(s,in_dim,k=7):
        super().__init__()
        s.k=k; s.in_dim=in_dim
    ...
    def forward(s,x):
        # x: (B,T,in_dim) -> (B,T,in_dim*(k+1))
        lin=(x*s.w_lin+s.b_lin).unsqueeze(-1)         # (B,T,in_dim,1)
        per=torch.sin(x.unsqueeze(-1)*s.W+s.B)         # (B,T,in_dim,k)
...
class GRUNet(nn.Module):
    """v8: GRU with Time2Vec for numerics + FiLM conditioning the final hidden state on
    a pooled context vector (sex + temporal summary). Letting time-series act as a condition."""
    def __init__(s):
        super().__init__()
        s.embs=nn.ModuleList([nn.Embedding(VOCAB[c],8,padding_idx=0) for c in CAT]+
                             [nn.Embedding(VOCAB['role'],4,padding_idx=0),
                              nn.Embedding(VOCAB['sex'], 4,padding_idx=0)])
        # Time2Vec for 3 numeric scalars: scoreSelf/10, scoreOther/10, strikeNumber/15
        s.t2v=Time2Vec(3,k=7)            # -> 3*(7+1) = 24 dim
        s.gru=nn.GRU(8*len(CAT)+4+4+24,64,batch_first=True)
        s.drop=nn.Dropout(0.2)
        # Context for FiLM: sex emb (4) + pooled t2v (24) + pooled role emb (4) = 32
    ...
    def forward(s,cat,num,ln):
        # categorical embeddings (each NCAT slice -> dim 8 or 4)
        emb_list=[s.embs[i](cat[:,:,i]) for i in range(NCAT)]    # NCAT tensors, each (B,T,emb_d)
        t2v=s.t2v(num)                                            # (B,T,24)
        e=torch.cat(emb_list+[t2v],-1)                            # (B,T, 8*7+4+4+24)
        # GRU body
        pk=nn.utils.rnn.pack_padded_sequence(e,ln.cpu(),batch_first=True,enforce_sorted=False)
        _,h=s.gru(pk); h=s.drop(h[-1])                            # (B, 64)
        # Build context: sex emb (last NCAT slice = sex) + mean-pooled t2v + mean-pooled role emb
        sex_emb=s.embs[NCAT-1](cat[:,0,NCAT-1])                   # (B,4) — sex constant per rally
        role_emb=s.embs[NCAT-2](cat[:,:,NCAT-2]).mean(1)          # (B,4) — mean over time
        # masked mean of t2v over valid positions
        mask=(torch.arange(t2v.size(1),device=t2v.device).unsqueeze(0)<ln.unsqueeze(1)).unsqueeze(-1).float()
        t2v_pool=(t2v*mask).sum(1)/mask.sum(1).clamp_min(1.0)     # (B,24)
        ctx=torch.cat([sex_emb,t2v_pool,role_emb],-1)             # (B,32)
        h=s.film(h,ctx)
...
np.savez('oof_v8_t2v_film.npz',LAc=LAc,LPc=LPc,LRc=LRc,TAc=TAc,TPc=TPc,TRc=TRc,GAc=GAc,GPc=GPc,GRc=GRc,YA=YA,YP=YP,YR=YR,SEEN=SEEN,RESCUED=RESCUED,COLD=COLD)
out("cached -> oof_v8_t2v_film.npz")
...
out(f"\nv8 T2V+FiLM CV-B: action F1={fa_b:.4f} w={WA} β={BA:.3f}  | CV-A F1={fa_a:.4f}")
...
out(f"v8 stats: aug players(action)={len(dMa_A)}  train-only players(point/server)={len(dMa_T)}  clusters aug={len(set(clA.values()))}/train-only={len(set(clT.values()))}")
