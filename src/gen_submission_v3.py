...
for f in range(5):
    trm=af!=f;evm=sf==f
    T=fit_trans({k:Xa.loc[trm,k].to_numpy() for k in KEY},yA[trm],yP[trm]); dMa,gA,dMp,gP=player_dists(nha[trm],yA[trm],yP[trm])
    cl=fit_clusters(tr[trfold!=f]); cA,cP=fit_matchup(nha[trm],lha[trm],yA[trm],yP[trm],cl)
    def mk(Xb,nh,lh,idx):
...
T=fit_trans({k:Xa[k].to_numpy() for k in KEY},yA,yP); dMa,gA,dMp,gP=player_dists(nha,yA,yP)
clF=fit_clusters(tr); cAF,cPF=fit_matchup(nha,lha,yA,yP,clF)
...
for tag,mask in [("incl0",False),("excl0",True)]:
    ...
    sub.to_csv(f"../submission_v3_matchup_{tag}.csv",index=False)
    out(f"[{tag}] rows={len(sub)} action={dict(sub.actionId.value_counts().head(3))} point={dict(sub.pointId.value_counts().head(3))}")
out(f"v3 submissions written {time.time()-t0:.0f}s")
print("V3DONE")
