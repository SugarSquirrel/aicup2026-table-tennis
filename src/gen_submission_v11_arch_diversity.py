...
class GRUNetV8(nn.Module):
    ...
    def forward(s,cat,num,ln):
        ...
        h=s.film(h,ctx)
        return s.ha(h),s.hp(h),s.hs(h).squeeze(1)
class GRUNetBiGRU(nn.Module):
    """v11: Bidirectional GRU — different inductive bias (sees future + past in seq)"""
    def __init__(s):
        super().__init__()
        s.embs=nn.ModuleList([nn.Embedding(VOCAB[c],8,padding_idx=0) for c in CAT]+[nn.Embedding(VOCAB['role'],4,padding_idx=0),nn.Embedding(VOCAB['sex'],4,padding_idx=0)])
        s.num=nn.Linear(3,16); s.gru=nn.GRU(8*len(CAT)+4+4+16,32,batch_first=True,bidirectional=True); s.drop=nn.Dropout(0.2)
        s.ha=nn.Linear(64,19); s.hp=nn.Linear(64,10); s.hs=nn.Linear(64,1)
    def forward(s,cat,num,ln):
        e=torch.cat([s.embs[i](cat[:,:,i]) for i in range(NCAT)]+[torch.relu(s.num(num))],-1)
        pk=nn.utils.rnn.pack_padded_sequence(e,ln.cpu(),batch_first=True,enforce_sorted=False)
        out_,h=s.gru(pk)
        # h shape: (2, B, 32) → concat to (B, 64)
        h_cat=torch.cat([h[0],h[1]],dim=-1); h_cat=s.drop(h_cat)
        return s.ha(h_cat),s.hp(h_cat),s.hs(h_cat).squeeze(1)
class GRUNetLSTM(nn.Module):
    """v11: LSTM — different gating mechanism than GRU"""
    def __init__(s):
        super().__init__()
        s.embs=nn.ModuleList([nn.Embedding(VOCAB[c],8,padding_idx=0) for c in CAT]+[nn.Embedding(VOCAB['role'],4,padding_idx=0),nn.Embedding(VOCAB['sex'],4,padding_idx=0)])
        s.num=nn.Linear(3,16); s.lstm=nn.LSTM(8*len(CAT)+4+4+16,64,batch_first=True); s.drop=nn.Dropout(0.2)
        s.ha=nn.Linear(64,19); s.hp=nn.Linear(64,10); s.hs=nn.Linear(64,1)
    def forward(s,cat,num,ln):
        e=torch.cat([s.embs[i](cat[:,:,i]) for i in range(NCAT)]+[torch.relu(s.num(num))],-1)
        pk=nn.utils.rnn.pack_padded_sequence(e,ln.cpu(),batch_first=True,enforce_sorted=False)
        _,(h,c)=s.lstm(pk); h=s.drop(h[-1])
        return s.ha(h),s.hp(h),s.hs(h).squeeze(1)
...
out(f"v11 (4-arch ensemble α={GRU_BLEND}) CV-B: action F1={fa_b:.4f} w={WA} β={BA:.3f}  | CV-A={fa_a:.4f}")
...
SEEDS=[42,1,2]  # 3 seeds per architecture
ARCH_WEIGHTS={'v7':0.35,'v8':0.25,'BiGRU':0.25,'LSTM':0.15}  # diversity-weighted (v7 best alone, gives most weight)
...
out(f"v11 arch diversity: training {len(SEEDS)} seeds × 4 architectures (v7,v8,BiGRU,LSTM)...")
ts0=time.time()
gA_v7,gP_v7,gR_v7=multiseed_train_predict(GRUNetV7,Ca,Na,La_,gyA,gyP,gyR,Cte,Nte,Lte,SEEDS); out(f"  v7    done in {time.time()-ts0:.0f}s")
ts0=time.time()
gA_v8,gP_v8,gR_v8=multiseed_train_predict(GRUNetV8,Ca,Na,La_,gyA,gyP,gyR,Cte,Nte,Lte,SEEDS); out(f"  v8    done in {time.time()-ts0:.0f}s")
ts0=time.time()
gA_bi,gP_bi,gR_bi=multiseed_train_predict(GRUNetBiGRU,Ca,Na,La_,gyA,gyP,gyR,Cte,Nte,Lte,SEEDS); out(f"  BiGRU done in {time.time()-ts0:.0f}s")
ts0=time.time()
gA_ls,gP_ls,gR_ls=multiseed_train_predict(GRUNetLSTM,Ca,Na,La_,gyA,gyP,gyR,Cte,Nte,Lte,SEEDS); out(f"  LSTM  done in {time.time()-ts0:.0f}s")
w=ARCH_WEIGHTS
gA_=w['v7']*gA_v7+w['v8']*gA_v8+w['BiGRU']*gA_bi+w['LSTM']*gA_ls
gP_=w['v7']*gP_v7+w['v8']*gP_v8+w['BiGRU']*gP_bi+w['LSTM']*gP_ls
gR_=w['v7']*gR_v7+w['v8']*gR_v8+w['BiGRU']*gR_bi+w['LSTM']*gR_ls
out(f"v11: 4-arch weighted ensemble: {w}, each arch=mean of {len(SEEDS)} seeds")
